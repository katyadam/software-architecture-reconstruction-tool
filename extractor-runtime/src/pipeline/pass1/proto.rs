...
mod tests {
    ...
    fn indexes_every_operation_with_rpc_bodies() {
        let record = extract_syntactic(
            "service ExampleService { rpc UnaryRpc (Request) returns (Response) {} rpc ClientStreamingRpc (stream Request) returns (Response) {} rpc ServerStreamingRpc (Request) returns (stream Response) {} rpc BidiStreamingRpc (stream Request) returns (stream Response) {} }",
            "example.proto",
        );
        assert_eq!(
            record.proto_services[0].operations,
