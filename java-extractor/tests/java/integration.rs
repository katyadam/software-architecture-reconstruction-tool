...
async fn should_return_empty_aggregate_for_empty_java_file() {
    ...
    assert!(agg.entities.is_empty(), "Empty file should have no entities");
    assert!(agg.endpoints.is_empty(), "Empty file should have no endpoints");
    assert!(agg.restcalls.is_empty(), "Empty file should have no REST calls");
    assert!(agg.callables.is_empty(), "Empty file should have no callables");
    assert!(agg.call_statements.is_empty(), "Empty file should have no call statements");
}
...
async fn should_parse_spring_controller_and_populate_aggregate_correctly() {
    ...
    assert_eq!(agg.endpoints.len(), 9, "FoodController should have 9 Spring endpoints");
    assert_eq!(agg.callables.len(), 10, "FoodController should have 10 callables (9 endpoint methods + dontCountThisMethod)");
    assert!(agg.restcalls.is_empty(), "Controller should have no outbound REST calls");
    assert!(!agg.imports.is_empty(), "FoodController should have imports");
...
async fn should_parse_restcall_service_and_populate_aggregate_correctly() {
    ...
    assert_eq!(agg.restcalls.len(), 7, "CancelServiceImpl should have 7 outbound REST calls");
    assert!(agg.endpoints.is_empty(), "Service class should have no endpoints");
    assert!(!agg.callables.is_empty(), "CancelServiceImpl should have callables");
}
